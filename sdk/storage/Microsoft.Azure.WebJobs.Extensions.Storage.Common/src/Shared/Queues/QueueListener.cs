            ...
            using (HttpPipeline.CreateClientRequestIdScope(clientRequestId))
            {
                try
                {
                    ...
                    if (_queueExists.Value)
                    {
                        ...
                        {
                            // if no messages can be fetched because we're at our limit or because
                            // throttles are enabled, wait a short period before checking again
                            return CreateDelayResult(TimeSpan.FromMilliseconds(50));
                        }
